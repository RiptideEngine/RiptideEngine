...
float4 psmain(const Fragment i) : SV_Target {
    ...
    const float distance = 0.5 - _MainTexture.Sample(_Sampler, i.uv);
    const float2 ddist = float2(ddx(distance), ddy(distance));
    
    const float pixelDist = distance / length(ddist);
