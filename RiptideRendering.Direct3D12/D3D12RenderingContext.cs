...
internal sealed unsafe partial class D3D12RenderingContext : BaseRenderingContext {
    ...
    public D3D12RenderingContext(ContextOptions options) {
        //Console.WriteLine(typeof(Vector2D<int>) == options.ComparisonType);

        Console.WriteLine(string.Join('\n', AppDomain.CurrentDomain.GetAssemblies().Select(x => x.FullName).GroupBy(x => x).Where(x => x.Count() > 1).SelectMany(x => x)));
