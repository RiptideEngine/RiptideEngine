...
internal unsafe sealed class UploadBufferPool(D3D12RenderingContext context) {
    ...
    private readonly List<AvailableEntry> _availables = [];
    ...
    public ID3D12Resource* Request(ulong minimumSize) {
        lock (_lock) {
            ...
            _ctx.Logger?.Log(LoggingType.Info, $"Direct3D12 - {nameof(UploadBufferPool)}: Resource created (Ptr = 0x{(nint)pOutput:X8}, Width = {rdesc.Width}, GPU Virtual Address = 0x{pOutput->GetGPUVirtualAddress():X8}).");
...
internal sealed unsafe class SuballocateUploadBuffer(UploadBufferPool pool) {
    ...
    private readonly UploadBufferPool _storage = pool;
